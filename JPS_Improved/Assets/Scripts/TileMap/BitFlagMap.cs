...
namespace FastPathFinder.Data
{
    ...
    public class BitFlagMap : IDisposable
    {
        ...
        public BitFlagMap(int width, int depth)
        {
            ...
            int quot = width / BIT_SIZE;    //몫
            int rem = width % BIT_SIZE;     //나머지
            ArrayWidthSize = quot;
        ...
        /// x, y 위치의 Flag 설정
        /// </summary>
        public bool SetFlag(int x, int y, bool value)
        {
            if (x < 0 || x >= Width) return false;
            if (y < 0 || y >= Height) return false;
        ...
        {
            return sizeof(UInt64) * _bitFlags.Length;
        }
        #endregion
    }
}
